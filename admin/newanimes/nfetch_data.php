...
if(isset($_POST['nview'])){
$sql="SELECT * FROM newanime";
...
if(mysqli_num_rows($result)>0){
	while ($row=mysqli_fetch_assoc($result)) {
			$output .="<tr class='anims'>
						...
						<td>".$row['time']."</td>
						<td><button class='btn btn-danger ndel' data-ndel='".$row['id']."'>Delete</button></td>
						<td><button class='btn btn-primary'><a href='nupdate.php?update=".$row['id']."'>Update</button></td>
					   </tr>";
	}
}
$output .="</table>";
